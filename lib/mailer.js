...
export async function sendProposalNotificationEmail(to, clientName, proposalNumber, proposalDate, grandTotal) {
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: process.env.MAIL_USER,
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px;">
        <h2 style="color: #333; margin-bottom: 20px;">Proposal Received</h2>
        
        <p style="color: #555; font-size: 16px;">Dear ${clientName || 'Valued Customer'},</p>
        
        <p style="color: #555; font-size: 16px;">
          We are pleased to inform you that a proposal has been sent to you. Please find the details below:
        </p>
        
        <div style="background-color: #ffffff; border: 1px solid #e0e0e0; border-radius: 6px; padding: 15px; margin: 20px 0;">
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 8px 0; color: #666;"><strong>Proposal Number:</strong></td>
              <td style="padding: 8px 0; color: #333;">${proposalNumber}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; color: #666;"><strong>Proposal Date:</strong></td>
        ...
        <p style="color: #555; font-size: 16px;">
          Please review the attached proposal PDF for detailed information. If you have any questions or would like to discuss further, please don't hesitate to contact us.
        </p>
  ...
  await transporter.sendMail({
    from: `"Indihands Admin" <${process.env.MAIL_USER}>`,
    to,
    subject: `Proposal ${proposalNumber} - Sent to You`,
    html: htmlContent,
  });
}
