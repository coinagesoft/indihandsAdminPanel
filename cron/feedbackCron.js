...
export function startFeedbackCron() {
  ...
  console.log("✅ Feedback Cron Started");
...
if (existing) {
    ...
    if (existing.email_sent_at) {
        ...
        console.log(
            `Already emailed: ${invoice.invoice_number}`
        );
    ...
    console.log(
        `Retrying email: ${invoice.invoice_number}`
    );
...
console.log(`Invoice updated: ${invoice.invoice_number}`);
  ...
  console.error(
    `Failed processing invoice ${invoice.invoice_number}`,
    err
  );
